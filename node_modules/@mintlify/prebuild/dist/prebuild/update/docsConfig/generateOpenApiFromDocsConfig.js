...
export const generateOpenApiFromDocsConfig = async (navigation, openApiFiles, pagesAcc, opts) => {
    ...
    async function processOpenApiInNav(nav) {
        ...
        if (openapi) {
            ...
            if (!openApiFile) {
                const invalidSpecFile = findInvalidSpecFile(invalidSpecFiles, openapi);
                throw new Error(invalidSpecFile
                    ? `Openapi file ${openapi} defined in ${getDivisionNav(nav)?.division} in your docs.json exists but is invalid: ${invalidSpecFile.reason}`
                    : `Openapi file ${openapi} defined in ${getDivisionNav(nav)?.division} in your docs.json does not exist`);
            }
            ...
            const processedPages = await Promise.all(newNav.pages.map(async (page) => {
                ...
                if (parsed) {
                    ...
                    if (!openapiPath) {
                        ...
                        if (!openApiFile) {
                            const invalidSpecFile = findInvalidSpecFile(invalidSpecFiles, openapiPath);
                            throw new Error(invalidSpecFile
                                ? `Openapi file ${openapiPath} referenced in docs.json exists but is invalid: ${invalidSpecFile.reason}`
                                : `Openapi file ${openapiPath} referenced in docs.json does not exist`);
                        }
